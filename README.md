...
首次保存完整连接信息后会显示「AI 工作台快速上手」。如果没有勾选「不再提示」，之后每次打开工作台都会再次显示这份基础操作说明。
...
`max_tokens` 和最大响应时间都可以选择「无上限」：前者不发送输出 Token 上限，后者不会因耗时自动取消。插件会按服务端返回的 `usage` 累计 Token，每增加一千万 Token 提醒一次；连续 300 秒没有收到响应数据时也会提醒，但请求仍会继续。
